...
func resourceRancherRegistryCredentialCreate(d *schema.ResourceData, meta interface{}) error {
	...
	registryCred := rancher.RegistryCredential{
		Name:        name,
...
func resourceRancherRegistryCredentialRead(d *schema.ResourceData, meta interface{}) error {
	...
	env, err := client.RegistryCredential.ById(d.Id())
	if err != nil {
		return err
	}

	log.Printf("[INFO] RegistryCredential Name: %s", env.Name)

	d.Set("description", env.Description)
	d.Set("name", env.Name)
	d.Set("email", env.Email)
	d.Set("public_value", env.PublicValue)
...
func resourceRancherRegistryCredentialDelete(d *schema.ResourceData, meta interface{}) error {
	...
	// Step 1: Deactivate
	if _, err := client.RegistryCredential.ActionDeactivate(reg); err != nil {
...
func RegistryCredentialStateRefreshFunc(client *rancher.RancherClient, registryCredID string) resource.StateRefreshFunc {
